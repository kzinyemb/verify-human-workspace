...
import { $getSelection, $isRangeSelection, COMMAND_PRIORITY_EDITOR, createCommand, $createParagraphNode } from 'lexical';
...
export function AutoPaginationPlugin() {
  ...
  useEffect(() => {
    ...
    // 🚨 Notice the new { dirtyElements, dirtyLeaves } extraction here
    const removeUpdateListener = editor.registerUpdateListener(({ dirtyElements, dirtyLeaves }) => {
      
      // 🚨 THE FIX: If no text was actually changed (e.g., you just clicked to place the cursor), 
      // exit the function immediately to prevent the DOM from reflowing and killing the caret!
